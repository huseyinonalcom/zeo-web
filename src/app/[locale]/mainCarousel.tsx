      ...
      <div className="relative hidden aspect-[45/17] w-full items-center justify-center overflow-hidden px-28 lg:flex">
        {items.map((item, index) => {
          ...
          return (
            <div
              key={item.name}
              className={`absolute transform transition-all duration-500 ease-in-out ${relativeIndex === 0 ? "z-10 w-1/2 cursor-default opacity-100" : relativeIndex == 1 ? "z-5 w-1/3 -translate-x-full opacity-50 cursor-pointer" : "z-5 w-1/3 translate-x-full opacity-50 cursor-pointer"} `}
